...
def strip_suffix(s):
	...
	if len(candidate_root) < 5:
		return candidate_root, the_suffix
...
def strip_prefix (s): ##usually, the prefix appears with a suffix e.g. paghandaan (pag ~~ an)		
	...
	return candidate_root, found, temp_prefix
#########################################
...
def main(args):
	...
	for words in wordfile.readlines():
		...
		the_reduplication = "_"
		root = words	
		temp_word, the_infix = strip_infix(words.lower())	 ###remove any infix first
		prefix_found=False
